...
class InitSpec extends BaseSpec with MockServer {
  ...
  Scenario("Can override copy on the business account entry page without overriding copy on the personal account entry page") {
    ...
    val journeyData: JourneyBuilderResponse = initializeJourneyV3(InitRequest(address = DEFAULT_BUSINESS_ADDRESS, messages = Some(Messages(en = english, cy = Some(welsh)))).asJsonString())
  ...
  Scenario("The sign out link is displayed when a sign out URL is supplied") {
    ...
    val journeyData: JourneyBuilderResponse = initializeJourneyV3(InitRequest(signOutUrl = Some(signOutURL)).asJsonString())
    startGGJourney(journeyData)
  ...
  Scenario("Cannot initialize a new journey with a maxCallCount but no maxCallCountRedirectUrl") {
    ...
    val thrown = intercept[Exception] {
      initializeJourneyV3("{\"serviceIdentifier\":\"bavf-acceptance-test\",\"continueUrl\":\"http://localhost:6001/complete\",\"messages\":{\"en\":{\"service.name\":\"bavf-acceptance-test\"}},\"bacsRequirements\":{\"directDebitRequired\":false,\"directCreditRequired\":false},\"maxCallConfig\":{\"count\":3}}")
    }
  ...
  Scenario("Cannot initialize a new journey with a maxCallCountRedirectUrl but no maxCallCount") {
    ...
    val thrown = intercept[Exception] {
      initializeJourneyV3("{\"serviceIdentifier\":\"bavf-acceptance-test\",\"continueUrl\":\"http://localhost:6001/complete\",\"messages\":{\"en\":{\"service.name\":\"bavf-acceptance-test\"}},\"bacsRequirements\":{\"directDebitRequired\":false,\"directCreditRequired\":false},\"maxCallConfig\":{\"redirectUrl\":\"http://localhost:6001/too/many/attempts\"}}")
    }
  ...
  Scenario("Cannot initialize a new journey with a maxCallCountRedirectUrl that redirects to an external resource") {
    ...
    val thrown = intercept[Exception] {
      initializeJourneyV3(InitRequest(maxCallConfig = Some(MaxCallConfig(3, "https://www.google.co.uk"))).asJsonString())
    }
