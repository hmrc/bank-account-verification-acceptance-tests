...
class BusinessAddressSpec extends BaseSpec with MockServer {
  ...
  Scenario("Business Bank Account Verification with address is successful") {
    ...
    val journeyData: JourneyBuilderResponse = initializeJourneyV1(InitRequest(address = DEFAULT_BUSINESS_ADDRESS).asJsonString())
  ...
  Scenario("Business Bank Account change is successful") {
    ...
    val journeyBuilderData: JourneyBuilderResponse = initializeJourneyV1(InitRequest(address = DEFAULT_BUSINESS_ADDRESS, prepopulatedData = Some(PrepopulatedData(accountType = "business"))).asJsonString())
    ...
    val initial: CompleteResponse = getDataCollectedByBAVFEV1(session.journeyId, journeyBuilderData.credId)
    ...
    val updated = getDataCollectedByBAVFEV1(session.journeyId, journeyBuilderData.credId)
  ...
  Scenario("Business Bank Account cannot be changed to an unknown bank") {
    ...
    val actual: CompleteResponse = getDataCollectedByBAVFEV1(session.journeyId, journeyBuilderData.credId)
