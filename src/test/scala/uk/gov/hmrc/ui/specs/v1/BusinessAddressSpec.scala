...
class BusinessAddressSpec extends BaseSpec with MockServer {
  ...
  Scenario("Business Bank Account Verification with address is successful") {
    ...
    val journeyData: JourneyBuilderResponse =
      journeyBuilder.initializeJourneyV1(InitRequest(address = DEFAULT_BUSINESS_ADDRESS).asJsonString())
  ...
  Scenario("Business Bank Account change is successful") {
    ...
    val journeyBuilderData: JourneyBuilderResponse = journeyBuilder.initializeJourneyV1(
      InitRequest(
    ...
    val initial: CompleteResponse =
      journeyBuilder.getDataCollectedByBAVFEV1(session.journeyId, journeyBuilderData.credId)
    ...
    val updated = journeyBuilder.getDataCollectedByBAVFEV1(session.journeyId, journeyBuilderData.credId)
