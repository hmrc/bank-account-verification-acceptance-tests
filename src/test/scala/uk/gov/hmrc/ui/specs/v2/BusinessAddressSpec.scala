...
package uk.gov.hmrc.ui.specs.v2
...
import uk.gov.hmrc.ui.models.response.v2.CompleteResponse
...
class BusinessAddressSpec extends BaseSpec with MockServer {
  ...
  Scenario("Business Bank Account Verification with address is successful") {
    ...
    val journeyData: JourneyBuilderResponse =
      journeyBuilder.initializeJourneyV2(InitRequest(address = DEFAULT_BUSINESS_ADDRESS).asJsonString())
    ...
    val actual: CompleteResponse = journeyBuilder.getDataCollectedByBAVFEV2(session.journeyId, journeyData.credId)
  ...
  Scenario("Business Bank Account Verification is successful with IBAN in response") {
    ...
    val journeyData: JourneyBuilderResponse = journeyBuilder.initializeJourneyV2()
  ...
  Scenario("Business Bank Account change is successful") {
    ...
    val journeyBuilderData: JourneyBuilderResponse = journeyBuilder.initializeJourneyV2(
      InitRequest(
    ...
    val initial: CompleteResponse =
      journeyBuilder.getDataCollectedByBAVFEV2(session.journeyId, journeyBuilderData.credId)
    ...
    val updated = journeyBuilder.getDataCollectedByBAVFEV2(session.journeyId, journeyBuilderData.credId)
  ...
  Scenario("Business Bank Account accepts only three tries when relevant configuration is set") {
    ...
    val session     = startGGJourney(journeyBuilder.initializeJourneyV2(initRequest))
