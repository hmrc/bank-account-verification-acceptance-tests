...
class PersonalAddressSpec extends BaseSpec with MockServer {
  ...
  Scenario("Personal Bank Account Verification with address is successful") {
    ...
    val journeyData: JourneyBuilderResponse =
      journeyBuilder.initializeJourneyV2(InitRequest(address = Some(DEFAULT_ADDRESS)).asJsonString())
  ...
  Scenario("Personal Bank Account change is successful") {
    ...
    val initial: CompleteResponse = journeyBuilder.getDataCollectedByBAVFEV2(session.journeyId, journeyData.credId)
    ...
    val updated: CompleteResponse = journeyBuilder.getDataCollectedByBAVFEV2(session.journeyId, journeyData.credId)
